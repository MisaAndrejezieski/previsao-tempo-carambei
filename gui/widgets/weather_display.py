...
class WeatherDisplay(tk.Frame):
    ...
    def atualizar(self, previsao: PrevisaoCompleta):
        ...
        self.text_area.insert(tk.END, "▬" * 45 + "\n", 'separador')
        self.text_area.insert(tk.END, "Dados: Open-Meteo\n", 'info')

    def _mostrar_clima_atual(self, atual):
        """Exibe apenas o clima atual."""
        condicao = atual.condicao
        self.text_area.insert(tk.END, "CLIMA ATUAL\n", 'subtitulo')
        self.text_area.insert(tk.END, f"{condicao.descricao}\n\n", 'destaque')
        self.text_area.insert(tk.END, f"Temperatura: {atual.temperatura}°C\n", 'neon_laranja')
        self.text_area.insert(tk.END, f"Sensação: {atual.sensacao_termica}°C\n", 'info')
        self.text_area.insert(tk.END, f"Umidade: {atual.umidade}%\n", 'info')
        self.text_area.insert(tk.END, f"Vento: {atual.vento_kmh} km/h\n", 'info')
        self.text_area.insert(tk.END, f"Precipitação: {atual.precipitacao} mm\n", 'info')
        self.text_area.insert(tk.END, f"Nuvens: {atual.nuvens}%\n", 'info')
    ...
    def _mostrar_previsao_7dias(self, previsao):
        ...
        for dia in previsao.previsao_7dias[:7]:
            ...
            self.text_area.insert(tk.END, f"{dia.condicao.descricao}\n", 'info')
            self.text_area.insert(tk.END, f"Máx: {dia.temp_max}°C  Mín: {dia.temp_min}°C\n", 'neon_laranja')
            if dia.precipitacao > 0:
                self.text_area.insert(tk.END, f"Precipitação: {dia.precipitacao} mm\n", 'info')
            else:
                self.text_area.insert(tk.END, "Sem chuva prevista\n", 'info')
    ...
    def mostrar_detalhes(self):
        ...
        if not self.previsao:
            self.text_area.delete(1.0, tk.END)
            self.text_area.insert(tk.END, "Nenhum dado disponível\n", 'erro')
            return
    ...
    def mostrar_erro(self, mensagem: str):
        ...
        self.text_area.delete(1.0, tk.END)
        self.text_area.insert(tk.END, f"Erro: {mensagem}\n", 'erro')